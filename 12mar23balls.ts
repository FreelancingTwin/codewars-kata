...
function ballProbability([ balls, [ a, b ], replace ]) {

  let ballLength = balls.length //int
  ,   blue = balls.filter(ball => ball === a).length //int
  ,   red = balls.filter(ball => ball === b).length; //int

  let result = blue / ballLength; //ex: 2/5
  ...
  return Math.round(result * red / ballLength * 1000) / 1000;
                    //(int / int * int / int * 1000)  / 1000
}